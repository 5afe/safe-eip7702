...
const Batch: React.FC = () => {
  ...
  const canExecute = () => !features.includes(FEATURES.SUPPORT_4337)
    || loading
    || !(safeStorage
      && account
      && safeStorage.owners?.includes(account.address) && safeStorage.threshold === 1n
      && safeStorage.modules?.includes(safeEIP7702Config[chainId].addresses.safe4337Module));
  ...
  return (
    <Grid container spacing={2}>
      ...
      {!features.includes(FEATURES.SUPPORT_4337) && (
        <Grid size={12}>
          <Alert severity="error">SUPPORT_4337 is not enabled in the features for this chain.</Alert>
        </Grid>
      )}
