...
function Delegate() {
  ...
  const [isWaitingForTransactionHash, setIsWaitingForTransactionHash] =
    useState<boolean>(false)
  const [isWaitingForTransactionReceipt, setIsWaitingForTransactionReceipt] =
    useState<boolean>(false)
  ...
  const [openDialogTransactionStatus, setOpenDialogTransactionStatus] =
    useState<boolean>(false)
  ...
  const validateOwners = (): boolean => {
    ...
    return (
      uniqueOwners.size === owners.length &&
      owners.every((owner) => isAddress(owner))
    )
  }
        ...
        const transactionReceipt = await publicClient.waitForTransactionReceipt(
          {
            hash: result.txHash,
            pollingInterval:
              parseInt(import.meta.env.VITE_TRANSACTION_POOLING_INTERVAL) ||
              12_000
          }
        )
    ...
    <Box>
          ...
          <Grid size={12} container key={index} spacing={2} alignItems="center">
            <Grid size={10}>
              <TextField
                ...
                error={
                  !isAddress(owner) ||
                  owners.indexOf(owner) !== owners.lastIndexOf(owner)
                }
                helperText={
                  (!isAddress(owner) && 'Invalid address') ||
                  (owners.indexOf(owner) !== owners.lastIndexOf(owner) &&
                    'Duplicate singer address')
                }
                sx={{
                  '& .MuiInputBase-input': {
        ...
        <div>
          {isProxyDeployed ? (
            ...
            <Alert severity="info" sx={{ marginTop: 2 }}>
              <Typography>
                Relayer will deploy proxy at address:{' '}
                <Typography component="code" sx={{ fontFamily: 'monospace' }}>
                  {getShortAddress(proxyAddress)}
                </Typography>
              </Typography>
      ...
      {isDelegatedToSafeAccount() && (
        <Alert
          severity="success"
          sx={{ marginTop: 2 }}
          action={<Link to={'/settings'}>View more</Link>}
        >
          <Typography>
            Account already delegated and uses Safe singleton:{' '}
            <Typography component="code" sx={{ fontFamily: 'monospace' }}>
      ...
      {delegatee && !isDelegatedToSafeAccount() && (
        ...
        >
          <Typography>
            Account already delegated to address:{' '}
            <Typography component="code" sx={{ fontFamily: 'monospace' }}>
      ...
      <Dialog
        open={openDialogTransactionStatus}
        onClose={() => setOpenDialogTransactionStatus(false)}
      >
        <Box sx={{ padding: 2 }}>
          <Grid
            container
              ...
              <Grid size={12}>
                <Typography align="center">
                  ...
                  <Link
                    target="_blank"
                    rel="noreferrer"
                    to={`${safeEIP7702Config[chainId].explorer}/tx/${transactionHash}`}
                  >
                ...
                <Grid size={12}>
                  <Alert severity="success">
                    <Typography sx={{ color: 'white' }}>
                      Transaction executed. EOA now can be used in Safe wallet.
                    </Typography>
                  </Alert>
                </Grid>
                <Grid
