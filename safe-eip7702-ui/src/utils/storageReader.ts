...
export const readModuleStorageSlot = async (
  client: PublicClient,
  account: Address,
  key: string
): Promise<Hex> => {
...
export const readOwnerStorageSlot = async (
  client: PublicClient,
  account: Address,
  key: string
): Promise<Hex> => {
...
export const readMappingStorage = async (
  ...
  const slot = keccak256(
    encodePacked(['bytes32', 'bytes32'], [paddedKey, baseSlot])
  )

  const value =
    (await client.getStorageAt({ address: account, slot })) || zeroHash
...
export const readStorage = async (
  ...
  const singleton = sliceHex(
    (await client.getStorageAt({ address: account, slot: toHex(0) })) ||
      zeroHash,
    12,
    32
  )

  const ownerCount = hexToBigInt(
    (await client.getStorageAt({ address: account, slot: toHex(3) })) ||
      zeroHash
  )
  const threshold = hexToBigInt(
    (await client.getStorageAt({ address: account, slot: toHex(4) })) ||
      zeroHash
  )
  const nonce = hexToBigInt(
    (await client.getStorageAt({ address: account, slot: toHex(5) })) ||
      zeroHash
  )
