...
import {
  ...
  getSetupData,
  GUARD_STORAGE_SLOT,
  readModuleStorageSlot,
  readOwnerStorageSlot
} from '../src/utils/safe'
...
describe('EIP7702', () => {
  const setupTests = deployments.createFixture(async ({ deployments }) => {
    ...
    const safeCompatibilityFallbackHandler =
    ...
    const safeEIP7702ProxyFactory: SafeEIP7702ProxyFactory =
      await getSafeEIP7702ProxyFactory()
    const delegatorSigningKey = new ethers.Wallet(
  ...
  const assertEmptyAccountStorage = async (account: AddressLike) => {
    ...
    expect(
      await provider.getStorage(account, FALLBACK_HANDLER_STORAGE_SLOT)
    ).to.equal(ethers.ZeroHash)
    expect(await provider.getStorage(account, GUARD_STORAGE_SLOT)).to.equal(
      ethers.ZeroHash
    )
    ...
    expect(
      await readModuleStorageSlot(provider, account, SENTINEL_ADDRESS)
    ).to.equal(ethers.ZeroHash)
    expect(
      await readOwnerStorageSlot(provider, account, SENTINEL_ADDRESS)
    ).to.equal(ethers.ZeroHash)
  }
  ...
  describe('Test SafeEIP7702Proxy', function () {
    it('Give authority to SafeEIP7702Proxy with EOA address itself as Safe owner', async () => {
      ...
      const ownerAddresses = await Promise.all(
        owners.map(async (owner): Promise<string> => await owner.getAddress())
      )
      ...
      const isContract =
        (await provider.getCode(proxyAddress)) === '0x' ? false : true

      if (!isContract) {
        await safeEIP7702ProxyFactory
          .connect(deployer)
      ...
      const authorizationList = getAuthorizationList(
        chainId,
        authNonce,
        delegatorSigningKey.privateKey,
        proxyAddress
      )
      ...
      expect(
        isAlreadyDelegated &&
          (await provider.getStorage(account, 4)) ==
      ...
      expect(
        await isAccountDelegatedToAddress(
          provider,
          await delegator.getAddress(),
          proxyAddress
        )
      ...
      const setupTxResponse = await relayer.sendTransaction({
      ...
      expect(
        await provider.getStorage(account, FALLBACK_HANDLER_STORAGE_SLOT)
      ).to.equal(ethers.zeroPadValue(fallbackHandlerAddress, 32))
      expect(await provider.getStorage(account, GUARD_STORAGE_SLOT)).to.equal(
        ethers.ZeroHash
      )
      // Singleton address
      expect(await provider.getStorage(account, 0)).to.equal(
        ethers.zeroPadValue(await safeSingleton.getAddress(), 32)
      )
      // Owner count
      expect(await provider.getStorage(account, 3)).to.equal(
        ethers.zeroPadValue('0x01', 32)
      )
      // Threshold
      expect(await provider.getStorage(account, 4)).to.equal(
        ethers.zeroPadValue('0x01', 32)
      )
      expect(
        await readModuleStorageSlot(provider, account, SENTINEL_ADDRESS)
      ...
      expect(
        await readOwnerStorageSlot(provider, account, SENTINEL_ADDRESS)
      ).to.equal(ethers.zeroPadValue(ownerAddresses[0], 32))
    ...
    it('Revoke authority and clear storage', async () => {
      const { delegator, clearStorageHelper, relayerSigningKey } =
        await setupTests()
      const provider = ethers.provider
      const relayer = new ethers.Wallet(relayerSigningKey.privateKey, provider)
      ...
      let encodedSignedTx = await getSignedTransaction(
        provider,
        relayerSigningKey,
        authorizationList
      )
      ...
      expect(codeAtEOA).to.equal(
        ethers.concat([
          ACCOUNT_CODE_PREFIX,
          await clearStorageHelper.getAddress()
        ])
      )

      const clearAccountStorage = await ethers.getContractAt(
        'ClearStorageHelper',
        await delegator.getAddress()
      )
      const txClearStorageResponse = await clearAccountStorage
        .connect(relayer)
        .clearSafeStorage()
    ...
    it('Clear storage using onRedelegation()', async () => {
      ...
      if (!isContract) {
        await safeEIP7702ProxyFactory
          .connect(deployer)
          .createProxyWithNonce(await safeSingleton.getAddress(), setupData, 0)
      }
      ...
      const txSetupDataReceipt = await (
      ...
      const interfaceOnRedelegation = new ethers.Interface([
        'function onRedelegation()'
      ])
      const data = interfaceOnRedelegation.encodeFunctionData(
        'onRedelegation',
        []
      )
      const txResponse = await execTransaction(
        relayer,
        owners,
        safe,
        account,
