...
import {
  encodeMultiSend,
  MetaTransaction
} from '@safe-global/safe-smart-account'
import { getSignature, getStorageSlot } from '../src/utils/safeLite'
...
describe('SafeLite', () => {
  ...
  describe('multisend', () => {
    it('Invoke multisend using relayer', async () => {
      const { accountWallet, safeLite, relayerSigningKey } = await setupTests()
      ...
      const calldata = safeLite.interface.encodeFunctionData('multiSend', [
        encodedTx,
        r,
        vs
      ])
      ...
      expect(await provider.getCode(accountAddress)).to.equal(
        ethers.concat(['0xef0100', await safeLite.getAddress()])
      )
      expect(
        BigInt(await provider.getStorage(accountAddress, getStorageSlot()))
      ).to.equal(1n)
    })

    it('Invoke multisend (self sponsor)', async () => {
      const { accountWallet, safeLite } = await setupTests()
      const provider = ethers.provider
      const accountAddress = await accountWallet.getAddress()
      ...
      const encodedSignedTx = await getSignedTransaction(
        provider,
        new SigningKey(accountWallet.privateKey),
        authorizationList,
        accountAddress,
        0,
        calldata,
      ...
      const accountBalanceAfter = await provider.getBalance(accountAddress)
      expect(txReceipt?.status === 1, 'Transaction failed')
      expect(accountBalanceAfter).to.be.lessThan(
        accountBalanceBefore - totalAmount - BigInt(txReceipt?.gasUsed || 0)
      )
  ...
  describe('isValidSignature', async () => {
    const setup = deployments.createFixture(async ({ deployments }) => {
      const { accountWallet, provider, safeLite, relayerSigningKey } =
        await setupTests()
    ...
    it('Should magic value when signature is valid', async () => {
      const { accountWallet } = await setup()
      ...
      expect(
        await safeLiteInstance.isValidSignature.staticCall(
          hash,
