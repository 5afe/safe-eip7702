...
export const execTransaction = async (
  ...
  const comparableArray = await Promise.all(
    wallets.map(async (x: Signer) => [await x.getAddress(), x])
  )
  comparableArray.sort((a, b) =>
    a[0]
      .toString()
      .localeCompare(b[0].toString(), 'en', { sensitivity: 'base' })
  )
...
export const getSetupData = (
  ...
  paymentToken: AddressLike = ethers.ZeroAddress,
  payment: BigNumberish = 0,
  paymentReceiver: AddressLike = ethers.ZeroAddress
): string => {
  ...
  return safeInterface.encodeFunctionData('setup', [
    ...
    module !== undefined
      ? safeModuleSetupInterface.encodeFunctionData('enableModules', [modules])
      : ethers.ZeroAddress,
