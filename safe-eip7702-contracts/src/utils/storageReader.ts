...
export const printAccountStorage = async (
  provider: Provider,
  account: AddressLike,
  safeSingleton: AddressLike | null
) => {
  ...
  if (
    safeSingleton &&
    (await provider.getCode(account)) ===
      ethers.concat([ACCOUNT_CODE_PREFIX, safeSingleton.toString()])
  ) {
    ...
    console.log(
      'modules: ',
      await safe.getModulesPaginated(
  ...
  console.log(
    'module pointer at sentinel address: ',
    await readModuleStorageSlot(provider, account, SENTINEL_ADDRESS)
  )
  console.log(
    'owner pointer at sentinel address: ',
